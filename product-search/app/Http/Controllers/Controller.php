...
class Controller extends BaseController
{
	...
	public function index(Request $request) : JsonResponse
    {
		...
		if ($request['ram_id']) $query->where('ram_id', $request['ram_id']);
		if ($request['hdd_id']) $query->where('hdd_id', $request['hdd_id']);
		...
		if ($request['storage_min'] && $request['storage_max']) {
			...
			if ($request['storage_min']) $query->where('storage','>',$request['storage_min']);
			if ($request['storage_min']) $query->where('storage','<',$request['storage_max']);
		}
    ...
    {
        return new JsonResponse($this->model->update($request->all(), $id));
    }
    ...
    {
	    return new JsonResponse($this->model->delete($id));
}
